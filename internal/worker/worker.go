...
func (w *Worker) ProcessProductImage(ctx context.Context, origKey string) error {
	log.Printf("Worker: Processing product image: %s", origKey)
...
func (w *Worker) ProcessS3Event(ctx context.Context, bucket, key string) error {
	log.Printf("Worker: Received S3 event for key: %s", key)
	...
	log.Printf("Worker: Key %s does not match any processing pattern", key)
	return nil
}
